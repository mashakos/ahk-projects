...
if JoystickNumber <= 0
{
    ...
    {
        XInput_Init()
        SetTimer, WatchGuide, 10  ; Monitor the state of the guide button.
    }
...
if JoystickNumber > 0
{
	...
	Hotkey, %JoystickPrefix%%ButtonLeftAlt%, ButtonLeft
	Hotkey, %JoystickPrefix%%ButtonRight%, ButtonRight
	Hotkey, %JoystickPrefix%%ButtonRightAlt%, ButtonRight
	...
	Loop
	{
		...
		GetKeyState, joy2, %JoystickNumber%joy2
		if joy2 = D
			Send, {Browser_Back}
		GetKeyState, joy1, %JoystickNumber%joy1
		if joy1 = D
			Send, {Enter}
...
{
    {
        GKeyToHoldDownPrev = %GKeyToHoldDown%
            if (State.wButtons & 0x0400)
                GKeyToHoldDown = Browser_Back
            else
                GKeyToHoldDown = 
